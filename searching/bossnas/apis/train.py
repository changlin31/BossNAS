...
from openselfsup.hooks import build_hook
from openselfsup.utils import get_root_logger, optimizers, print_log
from bossnas.hooks.optimizer_hook import DistOptimizerHook
from bossnas.runners.multi_stage_runner import MultiStageRunner
try:
    import apex
...
def multipath_batch_processor(model, data, train_mode, **kwargs):
    ...
    for idx, forward_singleop_online in enumerate(model_.forward_op_online):
        ...
        loss = model(**data, mode='train', idx=idx,
...
def train_model(model,
    ...
    model.update_interval = getattr(cfg, 'update_interval', 1)
    # start training
    if distributed:
        _dist_train(
...
def _dist_train(model, dataset, cfg, logger=None, timestamp=None, meta=None):
    ...
    if cfg.resume_from:
        runner.resume(cfg.resume_from, resume_optimizer='resume_optimizer' in cfg and cfg.resume_optimizer)
    elif cfg.load_from:
